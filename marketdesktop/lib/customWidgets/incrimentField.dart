...
/// The same can be read as a string from [NumberInputWithIncrementDecrementOwn.controller].
...
///
/// ![NumberInputWithIncrementDecrementOwn](https://raw.githubusercontent.com/Abhilash-Chandran/number_inc_dec/doc-assets/gifs/example1.gif)
///
/// {@endtemplate}

class NumberInputWithIncrementDecrementOwn extends StatefulWidget {
  ...
  /// Defaults to [_NumberInputWithIncrementDecrementOwnState._defaultValidator] validator
  ...
  @override
  _NumberInputWithIncrementDecrementOwnState createState() => _NumberInputWithIncrementDecrementOwnState();
}

class _NumberInputWithIncrementDecrementOwnState extends State<NumberInputWithIncrementDecrementOwn> {
  ...
  void initState() {
    ...
    _controller.text = widget.isInt ? widget.initialValue.toString() : widget.initialValue.toStringAsFixed(widget.fractionDigits);
  }
  ...
  void _onChanged(String newValue) {
    ...
    _debounceTimer = Timer(Duration(milliseconds: 2000), () {
      ...
      //   var parsedAndClamped = _clampAndUpdate(_tryParse(newValue), widget.enableMinMaxClamping);
  ...
  void _onSubmitted(String newValue) {
    var parsedAndClamped = _clampAndUpdate(_tryParse(newValue), widget.enableMinMaxClamping);
    if (this.widget.onSubmitted != null && _formFieldKey.currentState != null && _formFieldKey.currentState!.isValid) {
  ...
  String? _defaultValidator(String? value) {
    ...
    if (parsed == null && value != null && (value.isNotEmpty && value != '-')) {
    ...
    } else if (parsed != null && !widget.enableMinMaxClamping && (parsed < widget.min || parsed > widget.max)) {
      ...
      num candidate2 = candidate1 < parsed ? candidate1 + widget.incDecFactor : candidate1 - widget.incDecFactor;
      if (candidate2.isInfinite || candidate2 < widget.min || candidate2 > widget.max) {
  ...
  bool _stepMismatch(num newValue) {
    ...
    num remainder = (value - widget.incDecFactor * (value / widget.incDecFactor).roundToDouble()).abs();

    num acceptableError = widget.isInt ? 0 : widget.incDecFactor / 153;
    return remainder > acceptableError && remainder < (widget.incDecFactor - acceptableError);
  }

  num _roundByStep(num value) {
    return widget.min + ((value - widget.min) / widget.incDecFactor).round() * widget.incDecFactor;
  }
  ...
  num _clamByStep(num value) {
    ...
    if (clampedValue < widget.min || clampedValue > widget.max) return inRangeValue;
    return clampedValue;
  }
  ...
  Widget build(BuildContext context) {
    return Transform(
      transform: Matrix4.diagonal3Values(widget.scaleWidth, widget.scaleHeight, 1.0),
      child: Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            if (widget.buttonArrangement == ButtonArrangement.incLeftDecRight) _buildIncrementButton(),
            if (widget.buttonArrangement == ButtonArrangement.incRightDecLeft) _buildDecrementButton(),
            if (widget.buttonArrangement == ButtonArrangement.leftEnd) _buildColumnOfButtons(),
            Expanded(
              flex: 1,
              child: TextFormField(
                key: _formFieldKey,
            ...
            if (widget.buttonArrangement == ButtonArrangement.incLeftDecRight) _buildDecrementButton(),
            if (widget.buttonArrangement == ButtonArrangement.incRightDecLeft) _buildIncrementButton(),
            if (widget.buttonArrangement == ButtonArrangement.rightEnd) _buildColumnOfButtons(),
          ],
        ),
      ),
    );
  ...
  Widget _buildDecrementButton() {
    return Container(
      decoration: widget.decIconDecoration ??
          BoxDecoration(
            border:
                widget.buttonArrangement == ButtonArrangement.leftEnd || widget.buttonArrangement == ButtonArrangement.rightEnd
                    ? null
                    : Border(
  ...
  num _clampAndUpdate(var currentValue, bool clamp) {
    ...
    currentValue = clamp && currentValue != null ? _clamByStep(currentValue) : currentValue;
  ...
  String numToString(currentValue) {
    return (widget.isInt ? currentValue?.toString() : currentValue?.toStringAsFixed(widget.fractionDigits)) ??
        _controller.text ??
        '';
  }
}
